...
MainWindow.setWindowTitle("Asgore msyt tool 1.33v")
...
def TxtToMsyt(file_content):
    #file_content = Reverse(Un_Freeze(file_content), '', '', '', '', False)#
...
def script():
    ...
    if sheet_text.toPlainText():
        converted_text = Reverse(Un_Freeze(sheet_text.toPlainText()), '', '', '', '\n', False)
        Text_content = Text_content.replace(text_list[script.current_item-1], converted_text)  #لسبب ما لم يعمل التجميد والعكس هنا
            ...
            if item:
                for cell in range(2, len(text_table['A'])+1):
                    if text_table['A'+str(cell)].value == item:
                        sheet_text.setPlainText(text_table['B'+str(cell)].value+sheet_text.toPlainText())
